...
class Skripsi
{
    ...
    public function index()
    {
        ...
        $columns = ['nim', 'nama', 'nama_jurusan', 'judul', 'nilai', 'lembar_pengesahan', 'status'];
        ...
        $query = "SELECT * FROM dbo.v_skripsi WHERE 1=1";

        if (!empty($searchValue)) {
            $query .= " AND (nim LIKE '%$searchValue%' OR nama LIKE '%$searchValue%' OR nama_jurusan LIKE '%$searchValue%'  OR judul LIKE '%$searchValue%' OR nilai LIKE '%$searchValue%' OR lembar_pengesahan LIKE '%$searchValue%' OR status LIKE '%$searchValue%')";
        }
        ...
        $response = [
            ...
            "recordsTotal" => countData("dbo.v_skripsi") ?? 0,
    ...
    public function verifikasi()
    {
        ...
        $query = "UPDATE dbo.skripsi SET status = ? WHERE id_skripsi = ?";
        $params = [$status, $id];
...
$skripsi = new Skripsi($connection);

if (isset($_POST['action'])) {
    if ($_POST['action'] == 'index') echo $skripsi->index();
    if ($_POST['action'] == 'verifikasi') echo $skripsi->verifikasi();
}