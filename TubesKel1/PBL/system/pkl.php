...
class Pkl
{
    ...
    public function index()
    {
        ...
        $columns = ['nim', 'nama', 'nama_jurusan', 'setifikat_pkl', 'nilai_pkl', 'status'];
        ...
        $query = "SELECT * FROM dbo.v_pkl WHERE 1=1";

        if (!empty($searchValue)) {
            $query .= " AND (nim LIKE '%$searchValue%' OR nama LIKE '%$searchValue%' OR nama_jurusan LIKE '%$searchValue%'  OR sertifikat_pkl LIKE '%$searchValue%'  OR nilai_pkl LIKE '%$searchValue%' OR status LIKE '%$searchValue%')";
        }
        ...
        $response = [
            ...
            "recordsTotal" => countData("dbo.v_pkl") ?? 0,
    ...
    public function verifikasi()
    {
        ...
        $query = "UPDATE dbo.pkl SET status = ? WHERE id_pkl = ?";
        $params = [$status, $id];
...
$pkl = new Pkl($connection);

if (isset($_POST['action'])) {
    if ($_POST['action'] == 'index') echo $pkl->index();
    if ($_POST['action'] == 'verifikasi') echo $pkl->verifikasi();
}