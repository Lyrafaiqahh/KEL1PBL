...
class Kompen
{
    ...
    public function index()
    {
        ...
        $columns = ['nim', 'nama', 'nama_jurusan', 'file_kompen', 'status'];
        ...
        $query = "SELECT * FROM dbo.v_kompen WHERE 1=1";

        if (!empty($searchValue)) {
            $query .= " AND (nim LIKE '%$searchValue%' OR nama LIKE '%$searchValue%' OR nama_jurusan LIKE '%$searchValue%'  OR file_kompen LIKE '%$searchValue%' OR status LIKE '%$searchValue%')";
        }
        ...
        $response = [
            ...
            "recordsTotal" => countData("dbo.v_kompen") ?? 0,
    ...
    public function verifikasi()
    {
        ...
        $query = "UPDATE dbo.kompen SET status = ? WHERE id_kompen = ?";
        $params = [$status, $id];
...
$kompen = new Kompen($connection);

if (isset($_POST['action'])) {
    if ($_POST['action'] == 'index') echo $kompen->index();
    if ($_POST['action'] == 'verifikasi') echo $kompen->verifikasi();
}